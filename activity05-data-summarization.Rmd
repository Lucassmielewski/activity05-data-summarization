...
**Response**: It will be skewed right as most variable distributions involving money are skewed to the right. There always outliers of very large incomes, but the minimum income can only be 0.
...
**Response**: STEM majors have median earnings centered around a higher values, but should be less skewed as their maximum earnings are equal to the max earnings of the entire dataset. 
...
**Response**: Rather than show produces a tibble on the knitted document adding the knitr::kable code produces an easier to read table. 
